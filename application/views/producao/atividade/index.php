                    ...
                    <table data-item-id="datatableview" class="table display table-hover  table-striped ">
                        <thead>
                            <tr> 
                                ...
                                <th style="width:15%">Termino</th>
                                <th style="width:5%">Quantidade</th> 
                                <th style="width:5%">Custos</th>
                                <th style="width:5%"></th>
                            </tr>
                        </thead> 
                        <tbody>
                            <? foreach($rows as $r) { ?>
                            <tr> 
                                <td><?= $r->nm_atividade.'<br><small>'.$r->nm_etapa.'</small>'?></td>
                                <td><?=  date('d/m/Y', strtotime($r->dt_inicio)).'<br>'.$r->hr_inicio?></td>
                                <td><?= $r->dt_termino.'<br>'.$r->hr_termino?></td>
                                <td><?= $r->qt_produto?></td> 
                                <td>R$ <?= number_format($r->vl_custo,2,',','.')?></td>
                                <td>
                                    <div class="d-flex gap-3"> 
                                        <a href="<?= base_url($tela . $r->id_processo . '/'.$r->id.'') ?>" class="text-success" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="Editar">
                                            <i class="mdi mdi-pencil font-size-18"></i>
                                        </a>
                                        <a href="<?= base_url($tela. 'impressao/' . $r->id_processo . '/'.$r->id.'') ?>" target="_blank" class="text-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="Imprimir">
                                            <i class="mdi mdi-file-chart font-size-18"></i>
                                        </a>
                                    </div> 
                                </td> 
                            </tr>
                        ...
                        <div class="col-sm-12">
                            ...
                            <select name="Register[id_maquina_usina]"  class="form-control">
                                ...
                                <?php foreach ($maquina as $m) { ?>
                                    <option data-item-id="<?= $m->id_maquina_tipo ?>" value="<?= $m->id ?>"><?= $m->nm_maquina ?></option> 
                                <?php } ?>
                            </select>
                        </div>
                    </div>
